...
def visu_post(conn, id_usuario, id_post, OS, BROWSER, IP, horario=None):
    with conn.cursor() as cursor:
        try:
            ...
            cursor.execute("START TRANSACTION")
            cursor.execute("INSERT INTO viu_post (id_usuario,id_post,id_OS,id_browser,IP) VALUES (%d,%d,%d,%d,'%s')" % (id_usuario,id_post,os_id,browser_id,str(IP)))
            cursor.execute("COMMIT")
...
def passaro_imagem(conn):
    with conn.cursor(pymysql.cursors.DictCursor) as cursor:
        cursor.execute("SELECT post.Imagem,id_passaro  FROM post INNER JOIN menciona_passaro ON post.id_post = menciona_passaro.id_post ORDER BY post.Imagem")
        res = cursor.fetchall()
        return res

def add_bio(conn,id_usuario,nome,imagem,txt):
    with conn.cursor() as cursor:
        try:
            cursor.execute("START TRANSACTION")
            cursor.execute('CALL edit_bio(%s,%s,%s,%s)' , (nome,imagem,txt,id_usuario))
            cursor.execute("COMMIT")
        except Exception as e:
            print(e)
...
def dados_visu(conn):
    with conn.cursor() as cursor:
        cursor.execute("SELECT browser.nome,OS.nome, COUNT(IP) FROM viu_post INNER JOIN browser ON browser.id_browser = viu_post.id_browser INNER JOIN OS ON OS.id_OS = viu_post.id_OS GROUP BY OS.id_OS,browser.id_browser")
        res = cursor.fetchall()
        return res
